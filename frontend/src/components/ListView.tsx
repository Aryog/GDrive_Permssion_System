    ...
    <div className="space-y-2">
        ...
        <div className="flex flex-col space-y-2">
            {items.map(item => (
                <div
                    key={item.id}
                    className="flex items-center justify-between p-3 rounded-lg border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-purple-900/50 transition-colors duration-200 w-full"
                >
                    <button
                    ...
                    >
                        {item.type === 'folder' ? (
                            <Folder className="w-5 h-5 text-gray-400 dark:text-white mr-3 flex-shrink-0" />
                        ) : (
                            (item as File).mimeType?.startsWith('image/') ? (
                                <Image className="w-5 h-5 text-gray-400 dark:text-white mr-3 flex-shrink-0" />
                            ) : (
                                <FileText className="w-5 h-5 text-gray-400 dark:text-white mr-3 flex-shrink-0" />
                            )
                        )}
                    ...
                    <div className="flex items-center space-x-4 ml-4 flex-shrink-0">
                        {item.type === 'file' && (
                            <span className="text-sm text-gray-500 dark:text-white/80 w-20 text-right">
                                {formatFileSize((item as File).size)}
                            </span>
                        )}
                        <span className="text-sm text-gray-500 dark:text-white/80 w-24 text-right">
